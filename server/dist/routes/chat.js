...
router.get("/groups", auth_1.authMiddleware, async (req, res) => {
    try {
        const groups = await prisma_1.prisma.chatGroup.findMany({
...
router.get("/direct", auth_1.authMiddleware, async (req, res) => {
    const currentUserId = req.user.id;
    try {
        // Find all users the current user has chatted with
        const sentMessages = await prisma_1.prisma.chatMessage.findMany({
            where: { senderId: currentUserId, groupId: null },
            select: { receiverId: true },
        });
        const receivedMessages = await prisma_1.prisma.chatMessage.findMany({
        ...
        const partnersWithUnread = await Promise.all(partners.map(async (partner) => {
...
router.get("/search-users", auth_1.authMiddleware, async (req, res) => {
    const currentUserId = req.user.id;
...
// Post a new message
router.post("/messages", auth_1.authMiddleware, async (req, res) => {
    const currentUserId = req.user.id;
...
// Add community group
router.post("/admin/groups", auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        const group = await prisma_1.prisma.chatGroup.create({
            data: {
                name,
                description,
...
router.put("/admin/groups/:id", auth_1.authMiddleware, async (req, res) => {
    if (req.user.role !== "ADMIN") {
        return res.status(403).json({ error: "Forbidden" });
    }
    const { id } = req.params;
    ...
    try {
        ...
        const group = await prisma_1.prisma.chatGroup.update({
            where: { id },
            data: updateData,
        });
        res.json(group);
...
router.post("/admin/groups/:id/avatar", auth_1.authMiddleware, upload.single("avatar"), async (req, res) => {
    if (req.user.role !== "ADMIN") {
        return res.status(403).json({ error: "Forbidden" });
    }
    const { id } = req.params;
    ...
    try {
        const uploadResult = await new Promise((resolve, reject) => {
            const stream = cloudinary_1.v2.uploader.upload_stream({ resource_type: "image", folder: "trainitnow/group-avatars" }, (error, result) => { if (error)
                reject(error);
            else
                resolve(result); });
...
router.delete("/admin/groups/:id", auth_1.authMiddleware, async (req, res) => {
    if (req.user.role !== "ADMIN") {
        return res.status(403).json({ error: "Forbidden" });
    }
    const { id } = req.params;
    try {
        await prisma_1.prisma.chatGroup.delete({
            where: { id },
        });
