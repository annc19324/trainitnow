...
export default function AdminFlashcardsPage() {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      showToast(language === "vi" ? "Lỗi hệ thống khi xóa bộ thẻ!" : "Error deleting flashcard set!", "error");
    }
  };
          ...
          {sets.map((set) => (
            <div key={set.id} className="glass-panel" style={{ padding: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <Link to={`/flashcards/${set.id}/edit`} className="btn btn-secondary" title="Edit Set">
                  <Edit size={18} />
                </Link>
