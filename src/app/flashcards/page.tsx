...
export default function FlashcardsPage() {
  ...
  const fetchSets = async () => {
    try {
      const res = await fetch(`/api/flashcards?userId=${(session?.user as any)?.id}`);
      if (res.ok) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/flashcards/${newSet.id}/edit`);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!confirm(language === 'vi' ? 'Bạn có chắc muốn xóa bộ thẻ này?' : 'Are you sure you want to delete this set?')) return;
  ...
  if (status === "unauthenticated") {
    return (
      <div className="container" style={{ padding: "4rem 1rem", textAlign: "center" }}>
        ...
        <p>{language === 'vi' ? 'Bạn cần đăng nhập để sử dụng tính năng thẻ ghi nhớ.' : 'You need to log in to use flashcards.'}</p>
        <Link href="/login" className="btn btn-primary" style={{ marginTop: "1rem" }}>{t("nav.login")}</Link>
      </div>
    );
  }
            ...
            <div key={set.id} style={{ 
              ...
              <div style={{ marginTop: "auto", display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "1rem", borderTop: "1px solid var(--border-color)" }}>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <button 
                    onClick={(e) => { e.stopPropagation(); router.push(`/flashcards/${set.id}/edit`); }}
