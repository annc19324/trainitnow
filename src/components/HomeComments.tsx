...
export default function HomeComments() {
  ...
  const fetchComments = async (reset = false) => {
    try {
      ...
      const res = await fetch(`/api/comments?skip=${currentSkip}&take=${TAKE}`);
  ...
  return (
    <div className="glass-panel" style={{ padding: "2rem", marginTop: "4rem" }}>
      ...
      {session ? (
        ...
        <div style={{ padding: "1.5rem", background: "var(--bg-secondary)", borderRadius: "var(--radius-md)", textAlign: "center", marginBottom: "2rem", color: "var(--text-secondary)" }}>
          <p>{t("home.comments.loginReq") || "Please log in to leave a comment or review."}</p>
        </div>
      )}
